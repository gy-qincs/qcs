...
(function($, document) {
	...
	var panelBuffer = '<div class="mui-poppicker">\
		<div class="mui-poppicker-header">\
			<button class="mui-btn mui-poppicker-btn-cancel" style="border:0px;background:#eee;color:blue">取消</button>\
			<button class="mui-btn mui-btn-blue mui-poppicker-btn-ok" style="border:0px;background:#eee;color:blue">完成</button>\
...
(function($, document) {
	...
	var domBuffer = '<div class="mui-dtpicker" data-type="datetime">\
		<div class="mui-dtpicker-header">\
			<button data-id="btn-cancel" class="mui-btn" style="border:0px;background:#fbfcfc;font-size:17px;color:#1169c9;float: left;">取消</button>\
			<button class="mui-btn" style="border:0px;background:#fbfcfc;font-size:17px;color:#1169c9;">上路时间</button>\
			<button data-id="btn-ok" class="mui-btn mui-btn-blue" style="border:0px;background:#fbfcfc;font-size:17px;color:#1169c9;">完成</button>\
		</div>\
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		init: function(options) {
			...
			ui.y.addEventListener('change', function(obj) {
				var now = new Date();
				/**
				 * 选择其他年份设置年份列表为1-12月
				 * @author zhaocj@19pay.com.cn
				 * @time 2016-04-01 17:36:09
				 */
				if(obj.detail.item.value != now.getFullYear() && obj.detail.item.value != "1989"){
					// 选择年份修改月份列表
					options.customData.m = [{"text":"01月","value":"01"},
					{"text":"02月","value":"02"},
					{"text":"03月","value":"03"},
					{"text":"04月","value":"04"},
					{"text":"05月","value":"05"},
					...
					{"text":"12月","value":"12"}];
					self._createMonth();
				} else if(obj.detail.item.value == "1989"){
					options.customData.m = [{"text":"06月","value":"06"},
					{"text":"07月","value":"07"},
					{"text":"08月","value":"08"},
					{"text":"09月","value":"09"},
					{"text":"10月","value":"10"},
		...
		_create: function(options) {
			...
			options.buttons = options.buttons || ['取消', '完成'];
			options.type = options.type || 'datetime';
			options.customData = options.customData || {};
			self.options = options;
			var now = new Date();
			options.beginYear = options.beginYear || "1989";
			options.endYear = options.endYear || (now.getFullYear());
			var ui = self.ui;
			//设定label
			self._setLabels();
			self._setButtons();
